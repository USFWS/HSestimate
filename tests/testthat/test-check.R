...
test_that("tooManyDaysHunted works for WF, DV, SCRG, WK", {
  test_tmd <-
    dplyr::tibble(days_hunted = c(1:5, 10, 20, 40, 60, 100, 200, 300), )

  expect_true(
    "too_many_days" %in%
      suppressMessages(tooManyDaysHunted(test_tmd, REF_DAY_LIMIT_WF)$error2))

  expect_true(
    "too_many_days" %in%
      suppressMessages(tooManyDaysHunted(test_tmd, REF_DAY_LIMIT_DV)$error2))

  expect_true(
    "too_many_days" %in%
      suppressMessages(tooManyDaysHunted(test_tmd, REF_DAY_LIMIT_SCRG)$error2))

  expect_true(
    "too_many_days" %in%
      suppressMessages(tooManyDaysHunted(test_tmd, REF_DAY_LIMIT_WK)$error2))
})

test_that("tooManyDaysHuntedCR works for Alaska CR", {
  test_tmd <-
    dplyr::tibble(
  ...
  tmd_result <-
    suppressMessages(tooManyDaysHuntedCR(test_tmd)) |>
    filter(!is.na(error2))

  expect_true(nrow(tmd_result) > 0)
})

test_that("tooManyDaysHuntedCR works for North CR", {
  n_states <- list()
  for (i in seq_along(REF_STATES_CR_NORTH)) {
...
test_that("tooManyDaysHuntedCR works for North CR", {
  n_states <- list()
  for (i in seq_along(REF_STATES_CR_SOUTH)) {
    n_states[[i]] <- rep(REF_STATES_CR_SOUTH[i], 12)
  }

  test_tmd <-
    dplyr::tibble(sampled_state = unlist(n_states),
                  days_hunted =
                    rep(
                      c(1:5, 10, 20, 40, 60, 100, 200, 300),
                      length(REF_STATES_CR_SOUTH)
                    ))
