test_that("getPartySize creates party_size field", {
  test_party <-
    dplyr::tibble(retrieved = rep(2, 6), comment = rep(NA, 6))
...
test_that("recalcPartyBag recalculates retrieved over bag for parties", {
  ...
  expect_identical(
    recalcPartyBag(getPartySize(test_party))$retrieved2, c(7, 4, 3, 1))
})

test_that("recalcPartyBag does not recalc retrieved over bag for non-parties", {
  test_party <-
    dplyr::tibble(
  ...
  expect_identical(
    recalcPartyBag(getPartySize(test_party))$retrieved2, test_party$retrieved)
})
