...
.onAttach <- function(libname, pkgname = "HSestimate") {
  ...
  try({
    desc_url <-
      paste0(
        "https://raw.githubusercontent.com/USFWS/HSestimate/refs/heads/main/",
        "DESCRIPTION")
    ...
    if (!local_version %in% REF_RELEASES) {
      ...
      packageStartupMessage(
        ...
        season,
        " season.\nUpdate with pak::pak('USFWS/HSestimate')"
      )
      ...
      packageStartupMessage("HSestimate",
                            " v",
                            local_version,
                            "\nCompatible with ",
                            season,
