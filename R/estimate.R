...
#' Calculate all stats
#'
#' The internal \code{calcAllStats} function calculates all stats.
#'
...
#' Calculate variance
#'
#' The internal \code{calcFreq} function calculates variance and other values.
#'
  ...
  function(alldata_df, type) {
    ...
    alldata_df |> 
      mutate(
        ...
        q_active = 
          1 - !!sym(paste0("active_", type, "_hunter_mean")),                                 
        # q for successful hunter sample variance
        q_successful = 
          1 - !!sym(paste0("successful_", type, "_hunter_mean")),
        ...
        var_prop_active_hunters = 
          (!!sym(paste0("active_", type, "_hunter_mean")) * .data$q_active * .data$fpcf) / (.data$n_hunters - 1),   
        # Variance of proportion of successful hunters
        var_prop_successful_hunters = 
          (!!sym(paste0("successful_", type, "_hunter_mean")) * .data$q_successful * .data$fpcf) / (.data$n_hunters - 1),   
        ...
        # Count squared
        count_squared = 
          .data$stratum_count ** 2,
        # Numerator of state day variance
        var_t_days_hunted = 
          .data$count_squared * .data$var_lday,                
        # Numerator of state bag variance
        var_t_retrieved = 
          .data$count_squared * .data$var_lbag,                
        # Active hunters by stratum
        t_p_active_hunters = 
          .data$stratum_count * !!sym(paste0("active_", type, "_hunter_mean")),           
        # Note: estimates for unretrieved harvest should be expanded by active
        # hunters per stratum
        # Total down by stratum
        t_unretrieved = 
          !!sym(paste0("unretrieved_", type, "_mean")) * .data$t_p_active_hunters,    
        # Successful hunters by stratum
        t_p_successful_hunters = 
          .data$stratum_count * !!sym(paste0("successful_", type, "_hunter_mean")),       
        ...
        var_t_unretrieved = 
          ((!!sym(paste0("unretrieved_", type, "_mean")) * !!sym(paste0("unretrieved_", type, "_mean")) * .data$var_prop_active_hunters) + (!!sym(paste0("active_", type, "_hunter_mean")) * !!sym(paste0("active_", type, "_hunter_mean")) * .data$var_ldwn)) * .data$count_squared
      ) |> 
      select(
        c(
          "state",
          ...
          "var_ldwn",
          prop_active_hunters = 
            !!sym(paste0("active_", type, "_hunter_mean")),
          "var_prop_active_hunters",
          prop_successful_hunters = 
            !!sym(paste0("successful_", type, "_hunter_mean")),
          "var_prop_successful_hunters",
          "t_days_hunted",
          "t_retrieved",
          "t_unretrieved",
...
#' @importFrom dplyr rename
#'
#' @param strat_est_df Stratum estimates tibble
#' @param type Type of calculation to run, may be one of: 'Ducks', 'Geese',
#'   'Brant', 'SeaDucks', 'MODO', 'WWDO', 'SACR', 'Woodcock', 'Snipe', 'Coots',
#'   'Rails', 'Gallinules', 'BTPI'.
#'
