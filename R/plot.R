...
#' @param variable Variable to plot; should be one of "days_hunted", "retrieved"
#'   or "unretrieved"
...
boxPlotWWDO <-
  function(season_df, variable, y_limits = "auto") {
    ...
    season_df |>
      left_join(REF_STATES_WWDO_DF |>
                  select(-"state"), by = "sampled_state") |>
      ...
      labs(
        title =
          paste(
            "WWDO",
            str_to_title(stringr::str_replace(variable, "_", " ")),
            "(non-zero)"),
        x = "State",
